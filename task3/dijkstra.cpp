...
//    void deleteMemory(); //descructor
  ...
  //  int**  matrix; //input matrix
   ...
   // int ** distancesMatrix; //distance matrix
    ...
    int predecessorsMatrix[1000][1000];
    int distancesMatrix[1000][1000];
...
node getMinNode(vector<node> & q2){
    node minNode;
    int tmpDistance = INT_MAX;
    int index=0;
    int i = 0;
   // for(int i=0; i < nodes; i++){
    for(std::vector<node>::iterator it = q2.begin(); it != q2.end(); ++it){
    	if( (*it).distance < tmpDistance){
		tmpDistance = (*it).distance;
		minNode = *it;
		index = i;
	}	
	i++;
...
//#pragma acc declare create(matrix,nodes) 
//#pragma acc declare create(distancesMatrix) 
//#pragma acc declare create(predecessorsMatrix) 
...
void dijkstraAll(){
    //initialization
    int tmp = nodes;
    #pragma acc parallel num_gangs(500)
    for (int j = 0; j < tmp; j++){
